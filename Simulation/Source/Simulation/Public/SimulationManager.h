#pragma once // Alternativa a #ifndef/#define per evitare inclusioni multiple
...
class SIMULATION_API ASimulationManager : public AActor {
    GENERATED_BODY() // Per riflessione e serializzazione di UE
...
public:
    ...
    // Calcolo della accelerazione gravitazionale terrestre in un punto specifico
    FVector3d GetEarthGravityAccelerationAt(FVector3d SailLocationUU, double SailDistanceFromEarthKm) const;

    // Calcolo della pressione solare in un punto specifico
    ...
    UPROPERTY(EditAnywhere   , Category = "Simulation Parameters | Settings"        , meta = (DisplayName = "Moltiplicatore della Forza"       , ClampMin = "0.01", ClampMax = "100000000")) float                         ForceMultiplier            = 1.0f;
    ...
    UPROPERTY(EditAnywhere   , Category = "Simulation Parameters | Settings"        , meta = (DisplayName = "Mostra Orientamento Vela"                                                    )) bool                          ShowOrientationDebug       = false;
    ...
    // Puntatore statico al singleton
    static ASimulationManager* Instance;

protected:
    // Funzione iniziale chiamata all'inizio della simulazione
    virtual void BeginPlay() override;

    // Funzione finale chiamata alla fine della simulazione
